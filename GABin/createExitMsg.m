...
%   $Revision: 1.1.6.5.2.1 $  $Date: 2009/06/04 14:17:11 $

enableLinks = 0; %feature('hotlinks') && dispMsg && ~isdeployed;
...
if enableLinks
    ...
    conLocalOptTag = sprintf(linkCmd,'local_min_found');            % exitflag 1: nlconst,barrier
    conInitPtTag = sprintf(linkCmd,'did_not_move');                 % exitflag 100: barrier
    firstOrdOptTag = sprintf(linkCmd,'first_order_opt_con');        % exitflag 1,100: barrier,nlconst
    feasDirTag = sprintf(linkCmd,'feasible_directions');            % exitflag 1,100: barrier,nlconst
    ...
    conNormStepTag = sprintf(linkCmd,'norm_curr_step_fminconip');   % exitflag 2: barrier
    ...
    inFeasTag = sprintf(linkCmd,'no_feasible_point');               % exitflag -20,-21,-22,-24,-25: barrier,nlconst
    unboundedTag = sprintf(linkCmd,'appears_unbounded');            % exitflag -3: barrier
    ...
    objectiveLimitTag = sprintf(linkCmd,'objective_limit');         % exitflag -3: barrier
    ...
    constrTolTag = sprintf(linkCmd,'constraint_tolerance');         % All exitflags: barrier,nlconst
    tolConTag = sprintf(linkCmd,'tol_con');                         % All exitflags: barrier,nlconst
    ...
    [uncLocalOptTag,uncInitPtTag,normGradTag,gradSizeTag,normStepTag,uncPossMinTag, ...
        ...
        localSingularLSQTag,lsqPossMinTag,tolFunLSQTag,functionTolLSQTag,funcTolFsolveTag,tolFunFsolveTag] = deal('');
end
        ...
        switch caller
            case 'barrier'
                ...
                detailedMsg = sprintf(['\nOptimization completed: The final point is the initial point.\n', ...
                    ...
                    'the maximum constraint violation,\n%e, is less than %soptions.TolCon%s = %e.\n\n', ...
    ...
    case optimal % exitflag = 1
        switch caller
            ...
            case 'barrier'
                ...
                detailedMsg = sprintf(['\nOptimization completed: The %sfirst-order optimality measure%s, %e, ', ...
                    'is less than\n%soptions.TolFun%s = %e, and the maximum constraint violation,\n%e, is less ', ...
                    'than %soptions.TolCon%s = %e.\n\n', ...
                    'Optimization Metric                                                 Options\n', ...
                    'relative first-order optimality = %10.2e               TolFun = %7.0e (%s)\n', ...
                    'max(constraint violation) = %10.2e                     TolCon = %7.0e (%s)\n'],firstOrdOptTag,endLinkTag, ...
        ...
        switch caller
            case 'barrier'
                ...
                detailedMsg = sprintf(['\nOptimization stopped because the relative %snorm of the current step%s', ...
                    ', %e,\nis less than %soptions.TolX%s = %e, and the maximum constraint\n', ...
        ...
        switch caller
            case 'barrier'
                ...
                detailedMsg = sprintf(['\nOptimization stopped because the %sconstraint violations ', ...
                    'are locally stationary%s \nbut the maximum constraint violation,', ...
                    ' %e, exceeds\n%soptions.TolCon%s = %e.\n\n', ...
                    'Optimization Metric                                          Options\n', ...
    ...
    case infeasDeltaX % exitflag = -22
        switch caller
            case 'barrier'
                ...
                detailedMsg = sprintf(['\nOptimization stopped because the %snorm of the current step%s ', ...
                    ', %e,\nis less than %soptions.TolX%s = %e, but the maximum constraint\n', ...
    ...
    case unbounded % exitflag = -3
        switch caller
            case 'barrier'
                ...
                detailedMsg = sprintf(['\n%s stopped because the objective function value, %e, is\n', ...
                    'less than %soptions.ObjectiveLimit%s = %e, and the maximum constraint\n', ...
                    'violation, %e, is less than %soptions.TolCon%s = %e.\n\n', ...
                    'Optimization Metric                                       Options\n', ...
                    'objective value = %10.2e            ObjectiveLimit = %7.0e (%s)\n', ...
                    'max(constr violation) = %10.2e              TolCon = %7.0e (%s)\n'],messageInfo{1},messageInfo{2}, ...
...
% Create message for output structure. basicMsg and detailedMsg get
% concatenated so both will appear in the output structure.
if nargout > 0
    basicMsg = regexprep(basicMsg,'<<(\w+).*?>stopping criteria details</a>>', '');  % Remove <detail> link
    basicMsg = regexprep(basicMsg,'<(\w+).*?>', '');        % Remove open tags from basicMsg
    basicMsg = regexprep(basicMsg,'</a>', '');              % Remove ending tags
    detailedMsg = regexprep(detailedMsg,'<(\w+).*?>', '');  % Remove open tags from detailedMsg
    detailedMsg = regexprep(detailedMsg,'</a>', '');        % Remove ending tags
    if ~isempty(detailedMsg)
        details = 'Stopping criteria details:';  % Only separate messages by 'Detail:' if there is a detailedMsg
    else
        details = '';
    end
    outputMsg = sprintf('%s%s\n%s',basicMsg,details,detailedMsg);       % Concatenate strings
end
